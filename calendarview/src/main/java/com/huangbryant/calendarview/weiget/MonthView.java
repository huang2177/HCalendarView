...
public class MonthView extends ViewGroup {
    ...
    public void setDateList(List<DateBean> dates, int currentMonthDays) {
        ...
        for (int i = 0; i < dates.size(); i++) {
            ...
            if (item_layout != 0 && HCalendarViewAdapter != null) {
                view = LayoutInflater.from(mContext).inflate(item_layout, null);
                TextView[] views = HCalendarViewAdapter.convertView(view, date);
                solarDay = views[0];
                lunarDay = views[1];
            } else {
            ...
            if (!mAttrsBean.getOnlyShow()) {
                view.setOnClickListener(new OnClickListener() {
                    @Override
                    public void onClick(View v) {
                        int day = date.getSolar()[2];
                        HCalendarView calendarView = (HCalendarView) getParent();
